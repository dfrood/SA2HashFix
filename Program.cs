...
namespace SA2HashFix
{
    class Program
    {
        static void Main(string[] args)
        {
            if (args.Length == 0)
            {
                Console.WriteLine("Please drop an SA2 save file onto this application.");
            }
            else
            {
                bool found = false;
                List<byte> tempSave = new List<byte>();
                tempSave.AddRange(File.ReadAllBytes(args[0]));
                if (tempSave.Count == 0x10000)
                {
                    byte[] splitForChecksum = tempSave.Skip(0x3040).ToArray();
                    WriteChaoSaveChecksum(splitForChecksum, true);
                    List<byte> byteArray = new List<byte>();
                    byteArray.AddRange(tempSave.Take(0x3040).ToArray());
                    byteArray.AddRange(splitForChecksum);
                    File.WriteAllBytes(args[0], byteArray.ToArray());
                    found = true;
                    Console.WriteLine("Checksum has been written to Chao save file.");
                }
                if (tempSave.Count == 0x6000)
                {
                    bool PC = false;
                    if (!args[0].Contains(".bin") && args[0].Contains("SONIC2B__S")) { PC = true; }
                    File.WriteAllBytes(args[0], WriteSaveChecksum(tempSave.ToArray(), PC));
                    found = true;
        ...
        static byte[] WriteSaveChecksum(byte[] save, bool PC)
        {
            byte[] checksum = new byte[4];
            if (PC) { checksum = BitConverter.GetBytes(save.Skip(0x2844).ToArray().Select(x => (int)x).Sum()); }
            if (!PC) { checksum = BitConverter.GetBytes(save.Skip(0x2844).ToArray().Select(x => (int)x).Sum()).Reverse().ToArray(); }
